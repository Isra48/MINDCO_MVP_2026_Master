...
1. **Versión de Strapi — CRÍTICO.** La app fue construida contra **Strapi v4** (los mappers
   leen la forma `data[].attributes` y `image.data.attributes`). **Strapi Cloud freemium hoy
   crea proyectos en Strapi v5**, que devuelve la respuesta **aplanada** (sin `attributes`).
   Si el proyecto nuevo es v5, el fetch traerá datos pero la app los verá **vacíos**.
   - Opción A: crear el proyecto en **v4** si la cuenta lo permite.
   - Opción B: usar v5 y que yo ajuste los mappers/servicios a la forma plana (cambio pequeño,
     ~3 archivos). Dime y lo hago.
2. **Tras crear el Strapi nuevo**, solo necesitas pegar 2 valores en [app.json](app.json) → `expo.extra`:
   `STRAPI_URL` (la URL del proyecto, sin `/` final) y `STRAPI_API_TOKEN` (token read-only).
   `CONTENT_SOURCE` ya está en `"strapi"`.
3. **Seguridad:** el token actual está commiteado en `app.json`. Al cambiar de cuenta queda inservible,
   pero conviene mover esto a `app.config.js` + `process.env` para no volver a commitear secretos.
   Dime si quieres que lo refactorice.
...
Para `image` de cada clase, sube cualquier imagen (puedes usar fotos de yoga/meditación). Si no tienes archivos, deja `image` vacío.
...
Sube una imagen para cada una (paisaje/ciudad/yoga). Si no tienes, déjalo vacío.
...
- backgroundImage: una imagen o video de fondo (opcional).
